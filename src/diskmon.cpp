    ...
    proc_name_stat("/proc/diskstats")
    {
        ...
        while (Util::readSingleLine(proc_name_mount, &str, line++))
        {
            ...
            if (qi::parse(
                ...
                str.cend(),
                (
                qi::as_string[qi::lit("/dev/") >> +qi::alnum][bp::ref(devname) = qi::_1]
                >> qi::blank
                >> qi::as_string[+(qi::string("/") >> *(qi::char_ - (qi::lit('/') | qi::blank)))][bp::ref(fspath) = qi::_1] // NOLINT
                >> qi::blank
                >> *qi::char_
                )))  // NOLINT
            {
    ...
    void run()
    {
        ...
        while (ros::ok() && do_loop)
        {
            ...
            for (auto&& di : disk_infos)
            {
                ...
                float rate = static_cast<float>(si.available) / static_cast<float>(si.capacity) * 100.;
                ...
                di_msg.read_bps = static_cast<float>(read_blk - di.second.read_prev) * 512. / diff_sec;
                di_msg.write_bps = static_cast<float>(wrote_blk - di.second.write_prev) * 512. / diff_sec;
