        ...
        while (ros::ok() && do_loop)
        {
            ...
            while (Util::readSingleLine(proc_name, &str, line++))
            {
                if (qi::parse(
                    ...
                    qi::lit("MemTotal:")
                    >> +qi::blank
                    >> +qi::int_[bp::ref(real_total) = qi::_1]
                    >> +qi::blank
                    >> qi::lit("kB")
                    )))     // NOLINT
                {
                    ...
                    qi::lit("MemAvailable:")
                    >> +qi::blank
                    >> +qi::int_[bp::ref(real_free) = qi::_1]
                    >> +qi::blank
                    >> qi::lit("kB")
                    )))     // NOLINT
                {
                    ...
                    qi::lit("SwapTotal:")
                    >> +qi::blank
                    >> +qi::int_[bp::ref(swap_total) = qi::_1]
                    >> +qi::blank
                    >> qi::lit("kB")
                    )))     // NOLINT
                {
                    ...
                    qi::lit("SwapFree:")
                    >> +qi::blank
                    >> +qi::int_[bp::ref(swap_free) = qi::_1]
                    >> +qi::blank
                    >> qi::lit("kB")
                    )))     // NOLINT
                {
            ...
            msg_real.data = (1.f - static_cast<float>(real_free) / static_cast<float>(real_total)) * 100.f;
            std_msgs::Float32 msg_swap;
            msg_swap.data = (1.f - static_cast<float>(swap_free) / static_cast<float>(swap_total)) * 100.f;
            std_msgs::Float32 msg_total;
            msg_total.data = (1.f - static_cast<float>(real_free + swap_free)
                                / static_cast<float>(real_total + swap_total))
                                * 100.f;
