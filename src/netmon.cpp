...
public:
    ...
    do_loop(true)
    {
            ...
            if (family == AF_INET)
            {
                ...
                if (it == ips.end())
                {
                    ...
                    pub_netifs.insert(
                        std::make_pair(
                            ifname,
                            nh.advertise<sysmon_ros::netif>(
                                std::regex_replace(ifname, std::regex(R"(\W)"), "_"), 1)));
        ...
        for (ifa_ = ifaddr; ifa_ != NULL; ifa_ = ifa_->ifa_next)
        {
            ...
            {
                rtnl_link_stats *stats = static_cast<rtnl_link_stats *>(ifa_->ifa_data);
        ...
        while (ros::ok() && do_loop)
        {
            ...
            for (ifa_ = ifaddr; ifa_ != NULL; ifa_ = ifa_->ifa_next)
            {
                ...
                if (family == AF_PACKET && ifa_->ifa_data != NULL)
                {
                    ...
                    tx_bps = static_cast<double>(stats->tx_bytes - if_stats[ifname].tx_bytes)
                                * 8.0 / diff_sec;
                    rx_bps = static_cast<double>(stats->rx_bytes - if_stats[ifname].rx_bytes)
                                * 8.0 / diff_sec;
                    if (stats->tx_packets > if_stats[ifname].tx_packets)
                    {
                        tx_err_rate = static_cast<double>(stats->tx_errors - if_stats[ifname].tx_errors)
                                        / static_cast<double>(stats->tx_packets - if_stats[ifname].tx_packets)
                                        * 100.0;
                    }
                    if (stats->rx_packets > if_stats[ifname].rx_packets)
                    {
                        rx_err_rate = static_cast<double>(stats->rx_errors - if_stats[ifname].rx_errors)
                                        / static_cast<double>(stats->rx_packets - if_stats[ifname].rx_packets)
                                        * 100.0;
                    }
                    msg.if_name = ifname;
                    msg.ip = ips[ifname];
                    ...
                    ROS_DEBUG("%1.3lf %s\t\trate : %10d, %10d, %3.2f, %3.2f",
                                diff_sec, ifname.c_str(), tx_bps, rx_bps, tx_err_rate, rx_err_rate);
